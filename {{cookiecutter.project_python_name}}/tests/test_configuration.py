...
class TestConfiguration:
    ...
    def test_default_settings(self):
        ...
        with patch(
            "{{cookiecutter.project_python_name}}.settings.Settings.model_config",
            {"extra": "ignore", "env_prefix": "{{cookiecutter.project_python_name|upper}}_"},
        ):
            settings = Settings()

            assert settings.app_name == "{{cookiecutter.project_python_name}}"
    ...
    @patch("{{cookiecutter.project_python_name}}.settings.Path.home")
    ...
    def test_load_config_without_toml_file(self):
        ...
        with patch("{{cookiecutter.project_python_name}}.settings.Settings.model_config", {"toml_file": []}):
            settings = Settings()
    ...
    def test_load_config_error(self):
        ...
        with (  # noqa: SIM117
            ...
            patch(
                "{{cookiecutter.project_python_name}}.settings.Settings.__init__",
    ...
    def test_settings_field_descriptions(self):
        ...
        assert Settings.model_fields["default_output_format"].description == "Default output format"
        assert Settings.model_fields["enable_colors"].description == "Enable colored output"
    ...
    def test_settings_model_config(self):
        ...
        assert settings.model_config.get("env_prefix") == "{{cookiecutter.project_python_name|upper}}_"
    ...
    def test_settings_with_environment_variables(self):
        ...
        with (
            patch.dict(
                "os.environ",
                {
                    "{{cookiecutter.project_python_name|upper}}_YOUR_ENV_VAR": "value",
                },
            ),
            patch(
                "{{cookiecutter.project_python_name}}.settings.Settings.model_config",
    ...
    def test_settings_environment_variable_override(self):
        ...
        with (
            patch.dict(
                "os.environ",
                {
                    "{{cookiecutter.project_python_name|upper}}_DEFAULT_OUTPUT_FORMAT": "json",
    ...
    def test_settings_case_insensitive(self):
        ...
        with (
            patch.dict(
                "os.environ",
                {
                    "{{cookiecutter.project_python_name}}_default_output_format": "json",
                },
            ),
            patch(
                "{{cookiecutter.project_python_name}}.settings.Settings.model_config",
